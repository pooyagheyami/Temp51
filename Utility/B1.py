...
class EanBarCode:
   ...
   def getImage(self, value, height = 50, extension = "PNG"):
      ...
      draw.text((0, height-9), self.code[0], font=font, fill=0)
      
      # Draw first part of number
      draw.text((position+7, height-9), self.code[1:7], font=font, fill=0)
         
      # Draw second part of number
      draw.text((len(bits)/2+6+position, height-9), self.code[7:], font=font, fill=0)
      ...
      #im.save(self.code+"."+lower(extension), upper(extension))
      im.save(UTILITY_PATH+u'barcode'+"."+lower(extension), upper(extension))
